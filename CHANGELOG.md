## 1.3.0 (2026-06-10)

* feat(amazon): add marketplace plugin ([7485491](https://github.com/jlave-dev/agent-skills/commit/7485491))

## <small>1.2.1 (2026-06-09)</small>

* chore(subtractive-ui): refresh plugin metadata ([8c0eaa2](https://github.com/jlave-dev/agent-skills/commit/8c0eaa2))

## 1.2.0 (2026-06-09)

* feat(subtractive-ui): add marketplace plugin ([2cb7af7](https://github.com/jlave-dev/agent-skills/commit/2cb7af7))

## <small>1.1.3 (2026-06-09)</small>

* docs(amazon-shopping): generalize browser guidance ([fd0fdea](https://github.com/jlave-dev/agent-skills/commit/fd0fdea))

## <small>1.1.2 (2026-06-09)</small>

* chore: remove skill-level versioning ([718b9c8](https://github.com/jlave-dev/agent-skills/commit/718b9c8))

## <small>1.1.1 (2026-04-06)</small>

* chore: remove consult-codex skill ([d90a7ad](https://github.com/jlave-dev/agent-skills/commit/d90a7ad))

## 1.1.0 (2026-03-30)

* feat(consult-codex): add consult-codex skill ([b11fcf3](https://github.com/jlave-dev/agent-skills/commit/b11fcf3))

## <small>1.0.3 (2026-03-17)</small>

* fix: include all released commit types in changelog notes ([69418bb](https://github.com/jlave-dev/agent-skills/commit/69418bb))

## [1.0.2](https://github.com/jlave-dev/agent-skills/compare/v1.0.1...v1.0.2) (2026-03-17)

## [1.0.1](https://github.com/jlave-dev/agent-skills/compare/v1.0.0...v1.0.1) (2026-02-10)

# 1.0.0 (2026-02-10)


### Bug Fixes

* set initial skill version to 0.0.0 to match repo ([6ab930f](https://github.com/jlave-dev/agent-skills/commit/6ab930fb44b1e752d5c9a26e01a1ea73018cc78e))


### Features

* add amazon-shopping skill ([af7e393](https://github.com/jlave-dev/agent-skills/commit/af7e393c2525ca9138fb4953f43c890354ca198d))
* add semantic versioning automation with release-please ([e6caf51](https://github.com/jlave-dev/agent-skills/commit/e6caf51abfeca57c7d410bbed505311b58138734))
